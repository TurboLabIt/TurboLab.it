...
Entrambe le strategie normalizzano la "tipografia fine" ([Dictionary::FINE_TYPOGRAPHY_CHARS](https://github.com/TurboLabIt/TurboLab.it/blob/main/src/Service/Dictionary.php)): tutte le lineette tipografiche (trattini Unicode, barra orizzontale, segno meno) diventano `-`, le virgolette curve e gli pseudo-apostrofi (accento acuto, prime, ecc.) diventano dritti e il non-breaking space diventa uno spazio normale — qualunque sia la grafia in ingresso: carattere UTF-8, entity con nome o riferimento numerico. Il backtick e le virgolette basse « » (richieste per le citazioni) restano intatti. In coda, vengono corretti gli errori d'accento sempre-sbagliati: `E'` a inizio frase → `È`, la parola `po'` ricostruita da qualsiasi refuso (`pò`, `pó`, `po` nudo — mai `Po` maiuscolo né `.po`), accenti sbagliati (`perchè` → `perché`, `nè` → `né`, `quì` → `qui`, …), accenti mancanti su parole inequivocabili (`piu` → `più`, `citta` → `città`, …), la scorciatoia vocale+apostrofo (`PIU'` → `PIÙ`, `pubblicita'` → `pubblicità`, …), l'elisione sbagliata davanti ai maschili (`un'altro` → `un altro`: cade solo l'apostrofo, la parola resta com'è digitata; lista chiusa `TextProcessor::ELISION_MASCULINE_WORDS` di sole parole senza alcuna lettura femminile — `un'utente`, `un'assistente` e gli aggettivi epiceni come `un'utile opzione` restano intatti) e — regole editoriali — ogni variante di `menù` → `menu`, più i nomi propri editoriali `internet` → `Internet`, `pasqua` → `Pasqua`, `ferragosto` → `Ferragosto` e `capodanno` → `Capodanno` quando sono ben delimitati (mai in URL, percorsi o dentro `<code>`; `natale` resta fuori lista: è anche l'aggettivo di "città natale"). Le liste (`TextProcessor::ACCENT_MISSPELLED_WORDS` / `ACCENT_MISSING_WORDS` / `ACCENT_SURROGATE_WORDS`) sono rigorosamente opt-in, mai a regola generale: `lacchè`, `onestà`, `giacche`, il `retro`, il fiume `Po` e i `'DONE'` citati nel codice restano intatti. Anti-regressione: [FineTypographyNormalizationTest](https://github.com/TurboLabIt/TurboLab.it/blob/main/tests/Editor/FineTypographyNormalizationTest.php).

I blocchi di codice `<pre>` (pulsante `Codice` dell'editor, permessi in ogni articolo) sono byte-significativi. La normalizzazione tipografica si applica anche lì, di proposito: virgolette curve e nbsp dentro un comando sono danni da incolla, e un comando deve funzionare quando il lettore lo copia. Tutti gli altri fixer testuali — collasso dei doppi spazi (dentro `<pre>` sono l'indentazione!), correzioni d'accento (`velocita` può essere un identificatore, `E'...'` una stringa PostgreSQL), riparazioni legacy di `fixFormattingErrors()` (un blocco che inizia con `>>` diventerebbe `»`) — saltano i blocchi `<pre>` in blocco, tramite `HtmlProcessorBase::applyOutsidePreBlocks()`. HTMLPurifier invece vede **sempre** l'intero documento, `<pre>` compreso: la sanitizzazione non deve avere buchi. Il `<code>` interno può dichiarare il linguaggio (`class="language-*"`, evidenziato in pagina da highlight.js): i valori sono limitati da `Attr.AllowedClasses` a `HtmlProcessorForStorage::ALLOWED_CODE_LANGUAGES` — la stessa lista da cui il template costruisce il menu dell'editor (globale Twig `CodeLanguages` → `data-code-languages`), così l'editor non può offrire ciò che il purifier poi rimuoverebbe. Il default è nessuna classe = nessuna evidenziazione, e `class` resta un attributo del solo `<code>`. Anti-regressione: [CodeBlockStorageTest](https://github.com/TurboLabIt/TurboLab.it/blob/main/tests/Editor/CodeBlockStorageTest.php).
