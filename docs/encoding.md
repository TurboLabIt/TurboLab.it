...
Entrambe le strategie normalizzano la "tipografia fine" ([Dictionary::FINE_TYPOGRAPHY_CHARS](https://github.com/TurboLabIt/TurboLab.it/blob/main/src/Service/Dictionary.php)): tutte le lineette tipografiche (trattini Unicode, barra orizzontale, segno meno) diventano `-`, le virgolette curve e gli pseudo-apostrofi (accento acuto, prime, ecc.) diventano dritti e il non-breaking space diventa uno spazio normale — qualunque sia la grafia in ingresso: carattere UTF-8, entity con nome o riferimento numerico. Il backtick e le virgolette basse « » (richieste per le citazioni) restano intatti. In coda, vengono corretti gli errori d'accento sempre-sbagliati: `E'` a inizio frase → `È`, la parola `po'` ricostruita da qualsiasi refuso (`pò`, `pó`, `po` nudo — mai `Po` maiuscolo né `.po`), accenti sbagliati (`perchè` → `perché`, `nè` → `né`, `quì` → `qui`, …), accenti mancanti su parole inequivocabili (`piu` → `più`, `citta` → `città`, …), la scorciatoia vocale+apostrofo (`PIU'` → `PIÙ`, `pubblicita'` → `pubblicità`, …) e — regole editoriali — ogni variante di `menù` → `menu` e `internet` → `Internet` quando è ben delimitato (mai in URL, percorsi o dentro `<code>`). Le liste (`TextProcessor::ACCENT_MISSPELLED_WORDS` / `ACCENT_MISSING_WORDS` / `ACCENT_SURROGATE_WORDS`) sono rigorosamente opt-in, mai a regola generale: `lacchè`, `onestà`, `giacche`, il `retro`, il fiume `Po` e i `'DONE'` citati nel codice restano intatti. Anti-regressione: [FineTypographyNormalizationTest](https://github.com/TurboLabIt/TurboLab.it/blob/main/tests/Editor/FineTypographyNormalizationTest.php).
